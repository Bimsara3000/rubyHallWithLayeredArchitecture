...
public class UpdatePaymentViewController implements Initializable {
    ...
    void onClickUpdate(ActionEvent event) throws SQLException {
        ...
        if (guestName != null && paymentType != null && !date.equals("") && date.matches(isDate)) {
            try {
                boolean isSaved = paymentBO.update(new PaymentDTO(
                ...
                if (isSaved) {
                    ...
                    Stage stage = (Stage) cmbPaymentType.getScene().getWindow();
                    stage.close();
                } else {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        Platform.runLater(() -> {
                    ...
                    cmbGuestName.setValue(paymentTM.getGuestName());
                    cmbPaymentType.setValue(paymentTM.getPaymentType());
                    dPPayments.setValue(LocalDate.parse(paymentTM.getDate()));
                    lblPaymentAmount.setText(String.valueOf(paymentTM.getAmount()));
                });
